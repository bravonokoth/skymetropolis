...
const goalSchema = {
  ...
  properties: {
    ...
    buildingType: {
      type: Type.STRING,
      enum: [BuildingType.Residential, BuildingType.Commercial, BuildingType.Industrial, BuildingType.Park, BuildingType.Road, BuildingType.PowerPlant, BuildingType.WaterPump, BuildingType.MixedUse],
...
export const generateCityGoal = async (stats: CityStats, grid: Grid): Promise<AIGoal | null> => {
  ...
  const prompt = `You are the AI City Advisor for a simulation game. Based on the current city stats and weather, generate a challenging but achievable short-term goal for the player to help the city grow. Consider utility needs (Power/Water) if demand is high. Return JSON.`;
...
export const generateNewsEvent = async (stats: CityStats, recentAction: string | null): Promise<NewsItem | null> => {
  ...
  const context = `City Stats - Pop: ${stats.population}, Money: ${stats.money}, Day: ${stats.day}, Happiness: ${stats.happiness}, Pollution: ${stats.pollution}, Weather: ${stats.weather}, Power: ${stats.powerSupply}/${stats.powerDemand}, Water: ${stats.waterSupply}/${stats.waterDemand}. ${recentAction ? `Recent Action: ${recentAction}` : ''}`;
  const prompt = "Generate a very short, isometric-sim-city style news headline based on the city state and weather. Can be funny, cynical, or celebratory. Mention blackouts or droughts if utilities are low.";
